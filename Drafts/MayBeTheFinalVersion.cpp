...
void beginLoop()
{
  ...
  servoOn = ps2x.ButtonPressed(PSB_CIRCLE);   // Lý do ấn thì true vì nó sẽ tự động tắt chứ không cần ấn lần nữa mới tắt
...
void servo()   // Thank you Tô Duy An
{
  ...
  if(servoOn)
  {
    pwm.setPWM(SERVO_7, 0, 80);  // nếu dùng nhiều hơn 1 servo dùng code dưới
    //for(int i = 2; i<=7; i++) pwm.setPWM(i,0,80)
    endLoop(500);                // 0.5s là 90 độ
    pwm.setPWM(SERVO_7, 0, 510);  // nếu dùng nhiều hơn 1 servo dùng code dưới
    //for(int i = 2; i<=7; i++) pwm.setPWM(i,0,510)
    endLoop(600);                // 0.5s là 90 độ
